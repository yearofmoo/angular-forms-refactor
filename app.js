...
System.register("app", [], function() {
  ...
  var SurveyPreview = System.get("survey-common").default;
  var SurveyPreview = System.get("survey-preview").default;
  var SurveyEditor = System.get("survey-editor").default;
  var SurveyIndex = System.get("survey-index").default;
  var SurveyIndex = System.get("survey-routes").default;
  angular.module("MyApp", ['ngRoute', 'SurveyIndex', 'SurveyEditor', 'SurveyPreview', 'SurveyRoutes']);
  return {};
});

System.register("survey-common", [], function() {
  "use strict";
  var __moduleName = "survey-common";
  angular.module("SurveyCommon", []).factory('surveyCache', [function() {
    var self,
        PREFIX = 'surveyCache';
    ...
    return self = {
      ...
      set: function(id, data) {
        if (self.exists(id)) {
          data.id = id;
          $traceurRuntime.setProperty(collection, id, data);
          self.save();
        }
      },
      add: function(data) {
      ...
      remove: function(id) {
        if (self.exists(id)) {
          delete collection[$traceurRuntime.toProperty(id)];
          self.save();
        }
      },
      save: (function() {
...
System.register("survey-editor", [], function() {
  ...
  angular.module('SurveyEditor', ['ngMessages', 'SurveyCommon']).controller("SurveyEditorPageController", ["surveyCache", "$routeParams", "$location", function(surveyCache, $routeParams, $location) {
    ...
    return {
      ...
      controllerAs: 'ctrl',
      template: '<form name="surveyForm" ' + 'ng-submit="ctrl.handleSubmit(surveyForm.$valid)" ' + 'novalidate>' + '  <div ng-transclude></div>' + '</form>'
    };
...
System.register("survey-index", [], function() {
  "use strict";
  var __moduleName = "survey-index";
  angular.module("SurveyIndex", ['SurveyCommon']).controller('SurveyListController', ['surveyCache', function(surveyCache) {
    var self = this;
    this.entries = surveyCache.list();
    this.count = count(this.entries);
    this.removeSurvey = function(entry) {
      surveyCache.remove(entry.id);
      delete self.entries[$traceurRuntime.toProperty(entry.id)];
    };
    function count(col) {
      var count = 0;
      for (var i in col) {
...
System.register("survey-preview", [], function() {
  ...
  angular.forEach({
    ...
    studyPreviewModule.directive(directiveSelector, [function() {
      return {
...
System.register("survey-routes", [], function() {
  "use strict";
  var __moduleName = "survey-routes";
  angular.module("SurveyRoutes", []).config(['$routeProvider', function($routeProvider) {
    $routeProvider.when('/new', {
      controller: 'SurveyEditorPageController as pageCtrl',
      templateUrl: './survey-editor.html'
    });
    $routeProvider.when('/preview/:id', {templateUrl: './survey-preview.html'});
    $routeProvider.when('/', {templateUrl: './survey-index.html'});
